...
#tellraw @a[tag=is_admin] {"score":{"name": "$dy","objective": "temp"}}
...
execute store result entity @s Motion[0] double 0.0052 run scoreboard players get $dx temp
execute store result entity @s Motion[1] double 0.014 run scoreboard players get $temp0 temp
execute store result entity @s Motion[2] double 0.0052 run scoreboard players get $dz temp

scoreboard players set $damage temp 650

scoreboard players operation $distance temp *= $8 const
